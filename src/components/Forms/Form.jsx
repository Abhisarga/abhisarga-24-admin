...
function aahniks_recursiveSyntaxProcessor(key, value, variables) {
    ...
    if (key.includes('-')) {
        ...
        if (!isNumeric(c)) {
            ...
            index = Number(index)
            if (!variables[p][index - 1]) variables[p][index - 1] = {}
            variables[p][index - 1] = aahniks_recursiveSyntaxProcessor(
                cc,
                value,
                variables[p][index - 1]
            )
...
export function FormCard({ mutation, children }) {
    ...
    return (
        <>
            <form
                className="mb-0 mt-6 space-y-4 rounded-lg p-8 shadow-lg sm:p-6 lg:p-8 "
